...
//Program results in linear function which linear parameter will be used in citiroc_gaus_HG.C macro
...
void citiroc_peaks_HG(Int_t np=20) //works fine
{
    int nofr = 661; //number of first run in series
    ...
    TH1F *h = (TH1F*) hist[8][0]->Clone(); //chosing the histogram, hist[n][0] where n = 0...10, corrsesponds to HG = 1,5,...,63.
    
    //peak finding
    Int_t npeaks = np;
    Double_t par[3000];
    
    TCanvas *c1 = new TCanvas("HV30","HV30", 800,1600);
    c1->Divide(1,2);
    c1->cd(1);
    ...
    TH1 *hb = s->Background(h,17,"same"); // iterations of backgorund, more->flat
    ...
    Int_t nfound = s->Search(h,5,"",0.01);
    ...
    //c1->cd(2);
    int range = 6800;
   TF1 *fline = new TF1("fline","pol1",0,range); // range
   h->Fit("fline","qn"); 
...
//trying to set peak markers, fit and hist w/o bckrnd on the same c1
   ...
   {    
       ...
       if(fit->GetParameter(3*i+3) > 2900 && fit->GetParameter(3*i+3) < 6800) //adjust for specific case
       {
