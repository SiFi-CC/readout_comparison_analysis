...
void citiroc_spectroscopy_reader_HG(){
...
//     }
    ...
    TList *l = new TList();
    //std::unique_ptr<TFile> myFile( TFile::Open("histograms_hg.root", "RECREATE") );
    for(int i  = 0; i < 64; i++)
    {
        for(int j = 0; j<N; j++)
        {
            //myFile->WriteObject(&h[j][i],TString::Format("Run%d_ch%d",672+j,8+i));
            l->Add(h[j][i]);
        }
    }
    TFile *f = new TFile(TString::Format("histogram_HG_list_Run%d-%d", nofr,nofr+N), "RECREATE"); 
    l->Write("histlist_HG", TObject::kSingleKey);
    f->ls();
